...
//!
//! Reads are unrestricted; every write builder is allowlisted by the card's
//! [`FlashMap`]: page-addressed (type 0x0600) to the parameter block or the
//! primary firmware blocks, linear (type 0x1900) to the screen-size record only.
...
/// The E120's 64 KB parameter block (`config/cards/e120.toml`).
pub const PARAM_BLOCK: u8 = 0x07;

/// The E120's primary firmware blocks; the golden backup at
/// [`GOLDEN_BLOCK`] is outside every allowlist.
pub const FIRMWARE_BLOCKS: Range<u8> = 0x00..0x0b;

/// First block of the E120's golden backup image.
pub const GOLDEN_BLOCK: u8 = 0x20;

/// Flash address of the E120's screen-size record, reachable only through
/// the linear-address frames.
pub const SCREEN_RECORD_ADDR: u32 = 0x0007_f000;

pub const SCREEN_RECORD_LEN: usize = 256;

/// The blocks and addresses one card's writes are confined to. Every write
/// builder is a method here, so no frame can be built outside a map.
...
/// The E120's map, the values the frame tests pin.
...
impl FlashMap {
    ...
    /// Linear writes can reach firmware, so exactly one address is allowed.
    ///
    /// # Errors
