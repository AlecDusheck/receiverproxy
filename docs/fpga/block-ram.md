...
* **lane3 is zero in all 512 entries**, and **lane2 only ever holds 0–7**. So
  ...
  always zero (HIGH).
* **351 of 512 entries are used**, entries 0–350, **contiguous, with no zero
  entry inside the used region**. Entries 351–511 are all zero (HIGH).
...
> is in `analysis/fpga/ebr_map_16.53.txt` / `ebr_map_10.81.txt`, and the two
> Ethernet receive FIFOs, the two write banks and the output-stage buffer are
> identified in [pixel-write-path.md](pixel-write-path.md). What hides this
> from a naive trace is a decode trap: **EBR bel pins are not set-arc sinks**;
