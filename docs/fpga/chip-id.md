...
> produced the `0x14C` vs `0x0214` difference is open.

**Not resolved.** The gateware's handling of the 16-bit driver-chip id
...
The repo sends `0x014C` deliberately: `config/chips/sm16269.toml`
...
*independent* already-decoded flags. That part stands (MEDIUM).
...
prevent: a striking structural observation that is an artefact of the tool.
Details in `analysis/fpga/negative_results_and_method.txt`.
...
inputs), was not re-examined and is MEDIUM.
...
3. **Full netlist recovery**: LUT + FF + BRAM + routing → RTL, then simulate.
   ...
   Mind the tracing-reliability caveat in
...
2. **`0x014C` is very likely the id to send, and `0x0214` is not**
   ...
   **Practical consequence: stop sending `0x0214`.**
   ...
   correct.
3. **The full set of ids the gateware recognises is not known.**
