...
| [pinout.md](pinout.md) | The full 197-pin table with direction from the routing graph (not from `BASE_TYPE`, which is a degenerate decode), the two RGMII gigabit ports proven pin by pin, the SPI flash bank, the ~147 LED-side pins, and the board architecture that implies. |
...
| [output-stage.md](output-stage.md) | HUB75E connector and signal set, what the SM16269 family requires, the S-PWM structure, scan handling and the OneScanLen/CardScanLen arithmetic, the scan table (and why its all-zero bit times are normal), the pixel mapping, and a ranked list of hypotheses for why the panel does not render. |
...
| `scripts/` | every analysis script, including `repro.sh` |
