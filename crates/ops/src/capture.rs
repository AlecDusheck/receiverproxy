...
use rawlink::{read_pcap, PcapPacket};
...
pub fn describe(info: &protocol::DiscoveryInfo) -> String {
    let model = receivers::by_id(info.card_id).map_or("unknown", |m| m.name.as_str());
    format!(
        "receiver card #{}: id=0x{:02x} model={model} firmware={}.{:02} detected size {}x{}",
