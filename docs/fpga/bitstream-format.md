...
Everything needed to parse, validate and rebuild the vendor firmware images in
...
The `.hex` extension is a misnomer: the files are binary, not Intel HEX.
...
2. The command stream contains a `VERIFY_ID` command whose operand is
   `0x41111043`, which is the ECP5 25F IDCODE. `ecpunpack --idcode 0x41111043`
...
Header fields across all five images:
...
* `Bitstream CRC: 0x3474` is identical in all five images even though the
  contents differ completely, so it is **not** a checksum of the bitstream.
  What it is: **NOT RESOLVED**.
* The header date is not the version-number order. 10.81 is dated
...
| `0x17A` | 582 274 | frame data (7562 × 77) |
| `0x8E3FC` | `FF`… | pad |
...
The CRC is **CRC-16, polynomial `0x8005`, initial value 0, MSB-first, no input
...
The frame area therefore ends at `0x17A + 7562·77 = 0x8E3FC`. An earlier note
in this project put it at `0x8ED24`, which is the end of the **EBR** block,
not the frame area.
...
bit order is set **per field** by the database, not globally.
...
**Always check the field-to-frame-bit mapping in
