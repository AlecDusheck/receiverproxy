...
on 16.53 gives 1.6–1.8, camera noise); see
...
just renders garbage, which is a *useful* signal: it bypasses the host
...
* `card screen-size --set` refuses to write a record that reads as erased.
