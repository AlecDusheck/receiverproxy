...
rxp show test rgb --hold           # gradient | rows | border | rgb | white | calibrate
rxp show pattern calibrate --layout wall.json     # label every module of a wall
rxp show blank
rxp brightness 40
```
...
The table is generated by `rxp card models --markdown` from `config/cards/*.toml` and the derived module classes; a test keeps it current.
...
| 2 × 128x64 1/16, SM16269S (`config/panels/p25-2x128x64-chain.toml`) | ✅ | ⚠️ | ⚠️ | ⚠️ | ❌ |
| DP5525 | ⚠️ | ⚠️ | ⚠️ | ⚠️ | ❌ |
...
The ⚠️ chip rows are the 87 derived module classes in `config/panels/`, grouped by driver-chip family.
<!-- /tested -->
...
[chip]
library = "config/chips/sm16269s.toml"
...
- `[meta]` the evidence behind the values and how far they are trusted: `status` `verified` (driven on a bench; the notes name the card and firmware), `derived` (values taken from vendor configuration files; the notes say how many agreed) or `stub` (a placeholder not expected to work), the number of vendor files behind it and, for a derived spec, the share that agree and a few of them by name. Optional; a spec without it counts as derived from no file at all.
...
Chip libraries are in `config/chips/`, panel specs in `config/panels/`. Each file carries a `status`. 21 chip libraries and 87 panel specs are `derived`, generated by `scripts/corpus-mine.py` from the vendor config corpus; a chip library's header and a panel spec's `[meta]` table state how many files agreed. `config/chips/sm16269s.toml` and `config/panels/p25-128x64-sm16269s.toml` are the `verified` pair: the P2.5 128x64 SM16269S module driven on a Colorlight E120 running firmware 16.53. A derived file is a vendor default, not a measurement; use it as a starting point and check it against a vendor file for the exact module when one exists.

How the generator derives each record, and its limits, is in [docs/building-a-config.md](docs/building-a-config.md).

## Notes and evidence

[docs/README.md](docs/README.md) indexes the reference: formats, protocol, card internals, gateware, and the bench method. The generator is pinned to the card's day-one flash by `crates/rcvbp/tests/day_one.rs`. Developed against one Colorlight E120 on firmware 16.53 with one P2.5 128x64 SM16269S module, on macOS; Linux builds and lints for x86_64 and aarch64 but has not been run against a card.
