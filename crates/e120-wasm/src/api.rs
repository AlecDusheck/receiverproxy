...
use e120_rcvbp::record01::{View, LEN as RECORD01_LEN};
use e120_rcvbp::spec::PanelSpec;
use e120_rcvbp::{image, Rcvbp};
...
/// What `e120 config gen` writes, in memory. A block-7 build failure leaves
/// `block7` empty and puts the reason in `notes`; the `.rcvbp` and the pack
/// are still returned.
pub fn generate(spec_toml: &str) -> Result<Generated> {
    ...
    let g = spec.generate_with(&chip)?;
    let rcvbp = g.rcvbp.to_file_bytes()?;
    let (block7, notes) = match image::compile(&spec, &g) {
        Ok(b) => {
...
pub fn diff(a: &[u8], b: &[u8]) -> Result<Diff> {
    ...
    let types_a: Vec<u16> = fa
        .records
        .iter()
        .map(e120_rcvbp::Record::type_u16)
        .collect();
...
pub fn libraries() -> Libraries {
    ...
    let panels = embedded::PANELS
        ...
        .map(|&(path, toml)| Panel {
            ...
            name: toml_name(path, toml),
            toml,
            mined: path.starts_with("config/panels/mined/"),
        })
        .collect();
...
mod tests {
    ...
    fn cli_gen() -> PathBuf {
        let out = root().join("target/e120-wasm-test");
