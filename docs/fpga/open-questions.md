...
**Not resolved.** Ranked hypotheses with per-hypothesis experiments
...
1. Flash `build/p25-128x64-sm16269s-block7.bin`, fix the screen-size record,
   `reload-params --full`, `send-params`. The generated config has
...
**Not resolved** in the gateware, but it is the leading
...
**Not resolved:** which bank the raster reads, which the Ethernet writes,
...
**Not resolved.** The frame is right (`33 00`, `0x09` at payload+5,
...
**Not resolved as a set.** The bench strongly indicates **`0x014C` is
...
**Not resolved:** which of those pads is A, B, C, D, E, CLK, LAT or OE,
...
the BGA, or a clear photo of the connector traced out. A scan address line
should be driven by a small counter and CLK by a toggling flop;
`analysis/fpga/pad_driver_logic_16.53.tsv` has the per-pad driver logic to
start from.

### 2.2 What are the 34 bidirectional pins?

**Not resolved.** They are real (out-enable driven from fabric,
...
**Not resolved.** 351 used entries of 512, 5-bit opcode + 16-bit
...
**Not resolved.** The 256-byte pack arrives over Ethernet and lands
...
**Searchable:** `analysis/fpga/ebr_map_16.53.txt` records the driven pins,
clock, write gate and generator locations of **all 53** instantiated block
RAMs, so "which EBR holds the parameter pack" is a question you can pose
...
See [pixel-write-path.md §1](pixel-write-path.md) for the decode trap that
hides this (EBR pins are not set-arc sinks).
...
**Not resolved.** Record 0x01 carries a γ float at `+0x01C` (2.8
...
**Not resolved.** The whole DSP row is populated as MULT18X18D
...
**Not resolved**, and it matters for every flashing operation.
...
**Not resolved**, but sharply narrowed. It is **not** an ASCII string,
...
**Not resolved.** The marker at `0xAFFF8` is per-image
...
**Not resolved.** It is identical in all five images despite
