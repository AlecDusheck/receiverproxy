...
| Test-pattern "failure" caused by a concurrent `e120 show fill --hold` streamer overwriting the framebuffer during the sweep | a card-internal pattern written into the framebuffer would be overwritten by the streamer; the `0x0107` latch frames come from the streamer | superseded: on 10.81 the generator is inert with the wire quiet; on 16.53 it works. Sweep method: `pkill -f e120`, confirm no frames on the wire, then `e120 card test-sweep` with the supply logged |
