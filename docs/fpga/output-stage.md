...
> no runtime setting for this: the driver-chip protocol is in gateware.**
...
> [chip-protocol-microcode.md](chip-protocol-microcode.md).** There *is* an
...
named `SetGclkNumsOfChipControlByChipCustom`, so it carries **GCLK
...
**Verified:** our generated scan table is byte-identical to the
...
parsed): **`minOE = 1e-4` appears in 93 files in total and in 24 of the 29 that
...
* **Which top-edge pad carries which HUB75 control signal** (A–E vs CLK vs LAT
  vs OE). The **RGB data group is identified** (§7.1) and the control
...
* **Whether the FPGA drives HUB75 directly or through buffers.** All banks are
  ...
  a pad.
* **The actual on-wire waveform has never been measured.** This bench has a PSU and a
...
(HIGH) This is in tension with the fact that content *does* change the panel:
the two cannot both be simply true, which is why the concurrency explanation
deserves priority.
...
*For:* the boot and chip-sweep scripts of the time both left
...
largely excluded: the panel draws 2.8–4 A and shows per-pixel structure
