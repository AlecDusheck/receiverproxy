...
/// Generate a spec's configuration in memory: the `.rcvbp`, the basic pack,
/// the block-7 image and the sources report. `label` names the spec in the
/// report; `load` resolves `[chip].library`.
///
/// # Errors
/// Fails on an invalid spec or chip library. A block-7 build failure is not
/// an error here: `block7` is `None` and the reason is the last note.
pub fn generate(spec: &PanelSpec, label: &str, load: Loader) -> Result<GenOutputs> {
    let g = spec.generate_with(&spec.chip_library(load)?)?;
    let rcvbp = g.rcvbp.to_file_bytes()?;

    let mut notes = Vec::new();
    let block7 = match rcvbp::image::compile(spec, &g) {
        Ok(b) => {
...
pub fn gen_config(
    ...
    let mut g = generate(&spec, spec_path, load)?;
