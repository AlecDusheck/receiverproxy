...
| 0x0600 | 0x300 | module-position pack body (type 0x17, pack+5) | `GetDefaultModulePos` @ 0x1558b0: screen (MaxW x MaxH) tiled by the grid unit (record +0x057/+0x058 lo, +0x24E/+0x24F hi = 16x16); count at +0x005; 10-byte entries from +0x016: `[outer, inner, x BE, y BE, w BE, h BE]`, row-major; **all-zero when tiles > 64** (the seller's 256x384 wall = 384 tiles) | **non-zero: count 0x20, 32 entries** |
| 0x0900 | 0x100 | chip-register block (record 0x84 verbatim, pack+4) | written only if `IsMultiRegisterChip()` — true for chip 0x14C in LEDVISION 9.6 / iSet 7, evidently false in the older tool that produced the factory image (page is 0xFF). `ExchangeChipRegisterWhenColorChanged` @ 0x1ea370 may permute per-colour registers when the colour-swap byte (0xC6 here) is non-zero — NOT RESOLVED | optional (`boot.arm_at_boot`) |
...
| 0x0C00 | 0x100 | current exchange (`CRCVCurrentDataManager::GetCurrentExchangeParam`) | zeros observed; not transcribed (medium) | zeros |
...
| 0x6000 | 0x400 | scan table (`GetScanTable` @ 0x1eabc0 → `CalScanTalbeDefault` @ 0x14d710) | renderer `FieldTableToScanTable` @ 0x1d1c00 fully decoded: 48 entries `(level, 24-bit BE value)` = 16 scan lines x 3, then +0x39E scan mode (16), +0x39F nScan (16), +0x3A0 identity line order, +0x3C0 16 `(start,end)` pairs. The bit-time solver (`FromSegmentToFrameTime` @ 0x1d0a70) is NOT transcribed; its input `GetCardScanLenExtra` is 512 for the wall and 256 for one module | **carried verbatim; may need regeneration** |
