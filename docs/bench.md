...
| card | Colorlight E120, firmware 16.53. `e120 discover` reports the running version |
...
| camera | 30 fps webcam on the panel, 1920x1080, avfoundation device `E120_CAMERA` (default `0`) |

`bench.py` keeps its state in `/tmp/e120-trials/`: captures (`cap-NAME.jpg`,
`cap-NAME.rgb`), per-LED crops (`hi-NAME.png`), generated patterns (`pat/`)
and the panel crop (`crop.txt`; `E120_CROP` overrides it, built-in default
`420:750:1060:250`).
...
conditions become segments of one looping stream through `e120 show video
...
| `e120 discover` | firmware version, control area end coordinates | reports a healthy size while the control area is erased (`startX = 0xFFFF`); the EEPROM is checked with `scripts/flash-review.py` |
...
| `bench.py boot --spec S` | kill streamers, power-cycle (off, 4 s, on), wait for discovery (25 s timeout), settle 12 s (discovery answers before boot parameters have loaded; packs pushed earlier are lost), `e120 config send --spec S`, report the armed current |
...
* Flashing firmware erases the parameter block (block 0x07). The config is
  rewritten afterwards: `e120 flash restore-block build/<panel>-block7.bin
  --commit`, then `scripts/eeprom-restore.py --commit`, then a power-cycle;
  `e120 provision` does all of it.
