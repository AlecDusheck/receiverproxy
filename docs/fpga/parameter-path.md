...
How configuration reaches the card, what it stores where, and every constant
the gateware is known or suspected to compare against.

Artefacts: `analysis/fpga/basic-pack-fields.tsv` (the complete 256-byte pack
field table), `analysis/fpga/parameter-transports.tsv` (every wire transport
plus the flash boot-image region map), `analysis/fpga/constants-searched.tsv`
(every constant, tagged FOUND / NOT FOUND / NEVER SEARCHED).
...
**Two framing conventions coexist and this matters:**
...
and `SendRealTimePacks` @ `0x32cf40`), is **41 packs**:
...
| 6–21 | `0x03` | 0–15 | `0x300` ea | pixel sequence = record 0x03, the mapping |
| 22–25 | `0x1F` | 0–3 | `0x400` ea | void line (8-byte header) |
| 26–33 | `0x32` | 0–7 | `0x400` ea | anti-void line (8-byte header) |
...
EEPROM-backed screen-size record and is reachable only through the *linear*
frame type `0x1900`.

Firmware lives in blocks `0x00`–`0x0A` with a golden copy at `0x20`, written
...
The mechanism rests on one solid gateware fact: **`CCLK.MODE = USRMCLK` and
...
**Read the offset convention carefully.** The TSV's `body_off` is the offset
into the 256-byte body (= the boot-image offset, since the body starts at
image `0x0000`). `docs/record-0x01-fields.md` uses `pack_off = body_off + 4`,
because on the wire the body is preceded by the 4-byte header
`[0x05, 0x00, 0x00, sub]`. Two earlier documents in this project conflated
these; the TSV carries both columns to end that.

Highlights, all **HIGH**, re-verified byte-for-byte against
`card-dumps/primary-region.bin` at `0x70000`:

| body offset | field |
|---|---|
| `+0x00` | marker `0xA8` |
| `+0x01..0x03` | the `FF FF FF` head triple |
| `+0x04..0x05` | module geometry, **order swapped by line direction**: `[W, H/2]` for line_dir ≥ 2, `[H/2, W]` for line_dir < 2. Factory `20 80` = `[32, 128]`, so line_dir 0. |
| `+0x07` | scan denominator (`0x10` = 16) |
| `+0x08` | grey bits (`0x0E` = 14) |
| `+0x09` | serial clock, BE (`0x0008`) |
| `+0x0B` | **OneScanLen** BE = 256 |
| `+0x0D` | **CardScanLen** BE = 512 factory / 256 ours |
...
|---|---|---|---|
| Pixel mapping (record 0x03) | `0x3000` | type `0x03` ×16 | 4096 × 3 B |
...
| Anti-void-line counters | `0x1800` | type `0x32` ×8 | `0x1000` |
...
### What the gateware could plausibly derive itself
...
"write gamma table" path. So on an uncalibrated card the gateware must be
...
The search was thorough: every LUT4 INIT in all five images corrected for
constant-tied inputs, plus CCU2 carry chains, one-hot decoder groups, and the
microcode ROM. Method and full detail in [chip-id.md](chip-id.md#4-what-was-searched).
...
feeds the mode selector", which is tractable.
