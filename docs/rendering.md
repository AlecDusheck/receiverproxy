...
|---|---|---|---|---|
| driver-chip id | `0x14C` (vendor name SM16169SH) | `[chip] library` → `sm16269s.toml` | `factory.rs` record 0x84 equality; measured as the only id 16.53 arms the SM16269S outputs for | `0x0DE` (SM16169S, non-SH) and `0x0214` (the SM16269S's own id, a stub in every vendor build) never arm the outputs. The 20-byte chip-control block is the protocol descriptor ([chip-control-block.md](chip-control-block.md)); of its tails only the SH pattern `1,5,6` renders, `2/4/8` and `3/5/7` never arm |
...
| driver registers | the chip library's table | `config/chips/sm16269s.toml` (record 0x84) | `factory.rs` | the vendor "Default Parameter" set (`sm16269-defaults.toml`) renders worse and the LEDSetting 2.2.6 set saturates the panel. No register in the table changes the black floor |
