...
| [output-stage.md](output-stage.md) | HUB75E connector and signal set, what the SM16269 family requires, the S-PWM structure, scan handling and the OneScanLen/CardScanLen arithmetic, the scan table (its all-zero bit times are normal), the pixel mapping, the output stage in the netlist (96 RGB pins, the global blank and 2:1 source select, counter vs BRAM), and the diagnostic notes that go with it. |
...
histograms, decode scripts) and E120 flash dumps under
