<script lang="ts">
  ...
  // The scan is the caption.
  let { width, height, scan, size = 96, caption = true }: { width: number; height: number; scan: number; size?: number; caption?: boolean } = $props();
...
<figure>
  <svg width={boxW} height={boxH} viewBox="0 0 {boxW} {boxH}" role="img" aria-label="{w}x{h} module, 1/{scan} scan">
    <defs>
  ...
  {#if caption}<figcaption class="caption">{w}x{h} 1/{scan}</figcaption>{/if}
</figure>
