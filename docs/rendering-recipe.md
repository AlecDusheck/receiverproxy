...
| frame order | brightness, rows, **500 µs gap**, **3 latches** | one latch never starts the display; two renders but decays into noise and back on a ~10 s period (the stale second buffer page); three hold. Without the gap the card latches before the last row packet is stored and the bottom row flickers (judged by eye; invisible to the 30 fps camera). Every content command sends through `e120_driver::Wall::show` (`Timing` defaults); `E120_LATCHES`, `E120_LATCH_GAP_US`, `E120_ROW_GAP_US`, `E120_FRAME_MS` override for experiments. |
| raster | `rows` (64 packets of 128 px) | the double-width layouts placed content in the wrong rows; they are deleted (recoverable from git before 2026-09-02). |
