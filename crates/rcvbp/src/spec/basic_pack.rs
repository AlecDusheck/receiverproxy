...
/// The first pack byte, as `GetBasicParam` writes it.
const MARKER: u8 = 0xA8;

/// True when `body` is one page that opens with the marker and closes with
/// its own CRC: what a boot image read off a card holds at the pack offset.
#[must_use]
pub fn verify(body: &[u8]) -> bool {
    let Ok(body) = <&[u8; 256]>::try_from(body) else {
        return false;
    };
    body[0] == MARKER && body_crc(body).to_le_bytes() == body[CRC..]
}
